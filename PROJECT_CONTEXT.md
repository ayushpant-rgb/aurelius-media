...
- **Current Status:** Main pages built (Home, Services Hub, Service Sub-pages, Categories, Blog, Contact, Privacy Policy, Terms & Conditions, Resources Hub, Ebook Landing Pages). 36 specific service pages built with programmatic routing (including education-marketing, edtech-marketing, d2c-ecommerce-marketing, saas-marketing, ai-chatbots, and 4 SEO pillar pages: performance-marketing, growth-marketing, creative-services, ai-marketing-automation). 8 blog posts live (5 recently rewritten with in-depth research, data tables, and consistent structure including vibe coding expansion with 8 tool profiles; 1 new BOFU post "Why Your Meta Ads ROAS Sucks"). All core homepage sections complete with scroll animations and marquee testimonials. About page removed (301 redirect to `/#how-it-works`). Contact page complete with lead capture form, live Cal.com inline booking embed (https://cal.com/aureliusmedia/15min), and social links. Privacy Policy and Terms & Conditions pages added with footer links. Resources hub with 4 ebook landing pages (gated PDF downloads with lead capture). Newsletter subscription forms functional across Footer, Blog List, and Blog Post pages — storing subscribers in Supabase `newsletter_subscribers` table with welcome email via Resend. Resource/ebook pages unpublished pending redesign with proper cover art (code preserved, routes return 404). Lead capture popup (`LeadPopup.tsx` in root layout) with exit-intent + 8-second delay triggers, "We take on 3 new clients per quarter" positioning, fields: Name, Email, Phone, Website (optional), Monthly Ad Spend dropdown. Suppressed on /contact and /admin, 7-day cookie after dismiss/submit. Submits to /api/leads with source 'popup', sends confirmation + notification emails via Resend.
...
| `/services/real-estate-marketing-dubai` | Dubai Real Estate Marketing | Complete |
...
- **May 20, 2026:** Fixed PostHog analytics in production. Vercel env vars `NEXT_PUBLIC_POSTHOG_KEY` and `NEXT_PUBLIC_POSTHOG_HOST` had trailing newline characters baked into the bundle (`"https://us.i.posthog.com\n"`), causing every event POST to a malformed URL. Static assets (`us-assets.i.posthog.com`) still loaded — masking the issue — but `/i/v0/e/` and `/s/` ingestion endpoints were never reached. `PostHogProvider.tsx` now defensively `.trim()` both env vars at runtime. Redeployed; confirmed events posting with status 200.
- **May 20, 2026:** Deleted 5 blog posts that "kept getting published with every update" — they were real `.mdx` files sitting in `content/blog/`, no auto-generation script. Removed: `linkedin-marketing-india`, `how-to-choose-performance-marketing-agency`, `edtech-marketing-agency`, `best-meta-ads-agency-india`, `performance-marketing-guide`. Stripped dead-link callouts from 6 other posts (all linked to `/blog/performance-marketing-guide`) and 2 inline links in `performance-marketing-agency-cost-india` (kept prose). Added 5 permanent 308 redirects in `next.config.ts` to preserve SEO equity: → `/services/linkedin-ads`, → `/services/performance-marketing`, → `/blog/edtech-marketing`, → `/services/meta-ads`, → `/services/performance-marketing`. Total blog posts on site: 22.
- **June 3, 2026:** Published 3 research-backed posts from the MarketerMilk content audit: `chatgpt-ads-vs-google-ads-vs-meta-ads` (BOFU comparison, featured), `content-marketing-funnel-stages` (audit topic #10, title reworked toward "marketing funnel stages"), and `dark-funnel-buyer-journey` (audit topic #30, with an AI-as-dark-funnel angle). All use chalk-style Higgsfield covers; the ChatGPT one composites real OpenAI/Google Ads/Meta logos via Higgsfield reference images. **Fixed the recurring "drafts keep publishing" bug** in `src/lib/blog.ts`: `getAllSlugs()` now derives from `getAllPosts()` and `getPostBySlug()` returns `published`, so `published: false` posts truly 404 (previously only hidden from listing/sitemap while the URL still rendered live). The 4 intended drafts now 404. Also deployed the previously-uncommitted May 20 work and rewrote 6 posts' links off the `meta-ads-andromeda-update` draft. Total blog posts on site: 25.
- **June 3, 2026:** Added `how-to-measure-ai-search-visibility` — a measurement-focused companion to the `ai-seo-strategy` pillar (deliberately distinct angle + cross-linked to avoid keyword cannibalization). Used a **flat-vector explainer thumbnail** in the `edtech-marketing.webp` style (generated via Higgsfield with that image as a reference). Also replaced the cover on the high-traffic `what-works-on-instagram-2026` with a chalk-style cover featuring the real Instagram logo (Higgsfield reference); title/slug/meta left unchanged to protect rankings. Total blog posts on site: 26.
- **June 3, 2026:** Added `product-led-marketing` — "Product-Led Marketing: How to Sell Without Sounding Like Sales" (audit topic #41), a framework-driven original distinct from the Grow & Convert pain-point-copywriting source. Retitled from "product-led content" (source-blog title was taken) and framed as PLM = umbrella / PLC = engine; slug renamed `product-led-content` → `product-led-marketing` with a 308 redirect in `next.config.ts`. **Paper-collage thumbnail** in the `instagram-authors.jpeg` style (Higgsfield reference image) — a third cover style alongside chalk and flat-vector. Total blog posts on site: 27. Subsequently **expanded to a comprehensive ~4,600-word PLM strategy guide** (definition, PLM vs PLG, freemium/free-trial/reverse-trial models, the product-led funnel, PQLs, metrics, real examples, when it fails, TL;DR cheat sheet) synthesized from the ranking SERP — to match the depth of the top-ranking `what-works-on-instagram-2026` post and target rankings; category moved Content Strategy → Growth Marketing.
- **June 3, 2026:** Added `how-to-get-recommended-by-chatgpt` — BOFU/AEO post "How to Get Your Brand Recommended by ChatGPT" (~3.3k words, SERP-synthesized: consensus factor, the 9% problem, comparison queries, entity recognition, platform differences, audit playbook). Deliberately a **cluster page under the `ai-seo-strategy` pillar** (cross-linked, distinct recommendation-specific angle) to avoid cannibalization. Chalk cover with the ChatGPT/OpenAI logo (Higgsfield reference). Total blog posts on site: 28.
- **June 11, 2026:** Added Dubai Real Estate Marketing vertical landing page (`/services/real-estate-marketing-dubai`) targeting "real estate marketing agency dubai". Full V2 blueprint entry in `servicePages.ts` (off-plan launch campaigns, international investor source-market targeting, WhatsApp-first funnels, Golden Visa positioning, AED-denominated FAQs). Wired into Footer (Industries), sitemap.ts, services.ts, ServicePageClient icon/testimonial maps, and `serviceBlogCategoryMap`. The generic `real-estate-marketing` page now cross-links to the Dubai page via `relatedSlugs`. Also updated `serviceBlogCategoryMap` so education-marketing, edtech-marketing, d2c-ecommerce-marketing, and both real-estate pages match their vertical blog categories ("Education Marketing", "EdTech Marketing", "D2C Marketing", "Real Estate Marketing") for the Related Articles section.
- **June 3, 2026:** Added `meta-ads-bad-quality-leads` — "Why Your Meta Ads Are Generating Low-Quality Leads (and How to Fix It)" (~3.2k words). Retitled from a cruder "garbage" draft (slug unchanged → no redirect; cover regenerated as `-cover-v2.jpg`). Expanded into an 8-reason guide: Audience Network (#1, auto-enabled, ~67% fraud), bots/click fraud, optimizing for the wrong event, frictionless forms, no qualifying questions, loose targeting/weak creative, instant-form-vs-landing-page, slow follow-up — plus the CAPI lead-quality feedback loop and diagnostics. Distinct from `fix-meta-ads-roas` (cross-linked). Earned mid-article Aurelius lead-audit CTA (product-led). Chalk cover with the Meta logo (Higgsfield reference). Total blog posts on site: 29.
- **September 3, 2026:** Added `why-meta-ads-stop-working` — "Why Your Meta Ads Stop Working After a Few Days (It's Not Ad Fatigue)" (~6k words, multi-source consolidation via Firecrawl, not a single-source rewrite). Thesis: the 3-day drop is post-learning normalisation plus the relaunch loop, not fatigue; fix is five roles, 3:2:2 multi-asset ads, one CBO with Control/Test/Test, capped scaling steps and a two-question weekly diagnostic. Distinct from `fix-meta-ads-roas` (cross-linked). Chalk cover with the Meta logo (Higgsfield reference). Also: `blog/[slug]/page.tsx` renders `published: false` drafts on `next dev` only, for local preview. Total blog posts on site: 37.
