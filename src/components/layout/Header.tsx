...
export default function Header() {
    ...
    return (
        <>
            <header
                className={`fixed left-4 right-4 z-50 overflow-hidden transition-all duration-300 glass py-3 ${scrolled ? 'top-4' : 'top-4'}`}
            >
            ...
            {mobileOpen && (
                <div className="lg:hidden fixed inset-0 z-[60] bg-[#0B0B0D]/95 backdrop-blur-xl">
                    {/* Close button */}
                    <button
                        onClick={() => setMobileOpen(false)}
                        className="absolute top-5 right-5 p-2 text-brand-gray hover:text-brand-white transition-colors z-10"
